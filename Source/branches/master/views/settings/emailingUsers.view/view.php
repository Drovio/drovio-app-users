...
if (engine::isPost()) {
	...
	if ($_POST['send-username-update-confirmation-email'] == 'on') {
		$value = 1;
	} else {
		$value = 0;
	}
	$settings->set('send-username-update-confirmation-email', $value);
	if ($_POST['send-name-update-confirmation-email'] == 'on') {
		$value = 1;
	} else {
		$value = 0;
	}
	$settings->set('send-name-update-confirmation-email', $value);
	$settings->update();
...
$usernameUpdateEmail = $settings->get('SEND-USERNAME-UPDATE-CONFIRMATION-EMAIL');
$nameInfoUpdateEmail = $settings->get('SEND-NAME-UPDATE-CONFIRMATION-EMAIL');
...
$input = $form->getInput($type = "checkbox", $name = "send-name-update-confirmation-email", $value = "", $class = "", $autofocus = FALSE, $required = FALSE);
if ($nameInfoUpdateEmail == 1) {
	$input->setAttribute("checked", TRUE);
}
$frow = $form->buildRow("Send name update confirmation email", $input, $required = FALSE, $notes = "");
$form->append($frow);

$input = $form->getInput($type = "checkbox", $name = "send-username-update-confirmation-email", $value = "", $class = "", $autofocus = FALSE, $required = FALSE);
if ($usernameUpdateEmail == 1) {
	$input->setAttribute("checked", TRUE);
}
$frow = $form->buildRow("Send username update confirmation email", $input, $required = FALSE, $notes = "");
$form->append($frow);
