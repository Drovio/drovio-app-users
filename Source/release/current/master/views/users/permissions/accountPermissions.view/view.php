...
// Get all user groups
$allGroups = permissions::getAllGroups();

// Get account user groups
$accountID = engine::getVar("aid");
$accountGroups = permissions::getAccountGroups($accountID);
if (engine::isPost())
{
	...
	foreach ($allGroups as $groupID => $groupName)
	{
		...
		if (!isset($accountGroups[$groupID]) && isset($_POST['gid'][$groupID]))
			permissions::addAccountGroup($accountID, $groupID);
		
		if (isset($accountGroups[$groupID]) && !isset($_POST['gid'][$groupID]))
			permissions::removeAccountGroup($accountID, $groupID);
	}
	...
	{
		// Create new group
		permissions::addGroup($_POST['new_group']);
		
		// Refresh all groups
		$allGroups = permissions::getAllGroups();
		$newGroupID = array_search($_POST['new_group'], $allGroups);
		permissions::addAccountGroup($accountID, $newGroupID);
	}
