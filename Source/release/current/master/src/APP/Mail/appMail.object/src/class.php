...
class appMail
{
	...
	private static function sendTemplateMail($recipient, $subject, $templatePath_text, $templatePath_html, $attr = array())
	{
		...
		$mailgun = new mailgun($domain = "loginbox.io", $apikey = "api:key-b596b7499f9c28896ee15586ab0dbc65", $version = 3);
		
		// Set the sender to be from the current domain
		$teamName = "teamname";//team::getTeamUName();
		$mailgun->setFrom("no-reply@".$teamName.".loginbox.io");
		
		// Add recipient and send
		$mailgun->addRecipient($recipient);
		return $mailgun->send($subject, $mailContentsTEXT, $mailContentsHTML);
	}
}
//#section_end#
?>