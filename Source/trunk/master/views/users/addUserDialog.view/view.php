...
if (engine::isPost())
{
	...
	// Create account
	$status = account::getInstance()->create($email = $_POST['umail'], $firstname = $_POST['first_name'], $lastname = $_POST['last_name'], $password = $_POST['upass']);
	...
	$aid = $status;
	$accountInfo = account::getInstance()->info($aid);
	if ($_POST['action'] == 'welcome-email') {
		...
		$resetID = account::getInstance()->generateResetId($aid);
